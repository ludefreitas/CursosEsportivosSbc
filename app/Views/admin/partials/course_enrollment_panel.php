        ...
        ?>
            <article class="course-enrollment-row course-enrollment-status-<?php echo e((string) ($enrollment['status'] ?? '')); ?>">
                <div class="course-enrollment-person">
                    ...
                    <p><?php if ((int) ($enrollment['eh_pvs'] ?? 0) === 1 && trim((string) ($enrollment['numero_nis'] ?? '')) !== '') { ?><strong>Número do CadÚnico (NIS):</strong> <?php echo e((string) $enrollment['numero_nis']); ?> · <?php } ?><a href="#" class="course-future-link" data-future-label="PAR-Q">PAR-Q</a><?php if (!empty($enrollment['responsavel_nome'])) { ?> · <strong>Resp.:</strong> <?php echo e((string) $enrollment['responsavel_nome']); ?><?php } ?><?php if ($whatsapp !== '') { ?> · <?php if (!empty($professorView)) { ?><span class="course-whatsapp-link is-disabled" aria-disabled="true">WhatsApp: <?php echo e((string) (($enrollment['responsavel_whatsapp'] ?? '') ?: $enrollment['telefone_whatsapp'])); ?></span><?php } else { ?><a class="course-whatsapp-link" href="https://wa.me/<?php echo e($whatsapp); ?>" target="_blank" rel="noopener">WhatsApp: <?php echo e((string) (($enrollment['responsavel_whatsapp'] ?? '') ?: $enrollment['telefone_whatsapp'])); ?></a><?php } ?><?php } ?> · <button type="button" class="link-button course-address-open" data-address="<?php echo $jsonAttribute($address); ?>">Endereço</button></p>
                ...
                <div class="course-enrollment-links"><?php if (!empty($professorView)) { ?><span class="course-status-history-open status-current is-disabled" aria-disabled="true"><?php echo e((string) $enrollment['status_label']); ?></span><?php } else { ?><button type="button" class="course-status-history-open status-current" data-history="<?php echo $jsonAttribute((array) ($enrollment['historico'] ?? [])); ?>" data-enrollment-number="<?php echo e((string) $enrollment['id']); ?>"><?php echo e((string) $enrollment['status_label']); ?></button><?php } ?><button type="button" class="link-button course-enrollment-details-open" data-details="<?php echo $jsonAttribute($details); ?>">Detalhes</button></div>
                <div class="course-enrollment-next-action">
                    <?php if ((string) ($enrollment['status'] ?? '') === 'lista_espera' && $whatsapp !== '') { ?><?php if (!empty($professorView)) { ?><span class="btn btn-whatsapp course-vacancy-whatsapp is-disabled" aria-disabled="true">Informar vaga pelo WhatsApp</span><?php } else { ?><a class="btn btn-whatsapp course-vacancy-whatsapp" href="https://wa.me/<?php echo e($whatsapp); ?>?text=<?php echo e(rawurlencode($vacancyMessage)); ?>" target="_blank" rel="noopener">Informar vaga pelo WhatsApp</a><?php } ?><?php } ?>
                    <?php if ($nextStatus !== null) { ?><?php if (!empty($professorView)) { ?><span class="course-status-change-open is-disabled" aria-disabled="true"><?php echo e((string) $nextStatus['action']); ?></span><?php } else { ?><button type="button" class="link-button course-status-change-open" data-enrollment-id="<?php echo e((string) $enrollment['id']); ?>" data-enrollment-number="<?php echo e((string) $enrollment['id']); ?>" data-current-status="<?php echo e((string) $enrollment['status']); ?>" data-next-status="<?php echo e((string) $nextStatus['value']); ?>" data-next-label="<?php echo e((string) $nextStatus['label']); ?>"><?php echo e((string) $nextStatus['action']); ?></button><?php } ?><?php } else { ?><span class="muted">Sem próxima alteração</span><?php } ?>
                </div>
            </article>
        <?php } ?>
        </div>
