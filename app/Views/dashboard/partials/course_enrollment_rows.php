        ...
        <div class="table-wrap"><table class="data-table"><thead><tr><th>Ordem</th><th>Pessoa</th><th>Turma</th><th>Temporada</th><th>Status e orientações</th><th>Ação</th></tr></thead><tbody>
            <?php foreach ($courseEnrollments as $enrollment) { ?>
                ...
                $enrollmentDetails = [
                    ...
                    'nascimento' => format_birth_date_with_age($enrollment['data_nascimento'] ?? null),
                ...
                <tr data-dashboard-enrollment-id="<?php echo e((string) ($enrollment['id'] ?? 0)); ?>"><td><?php echo e((string) ($enrollment['numero_ordem'] ?? '-')); ?></td><td><?php echo e($enrollment['nome_completo']); ?><br><small class="muted"><?php echo e(format_birth_date_with_age($enrollment['data_nascimento'] ?? null)); ?></small></td><td><?php echo e($enrollment['turma_nome']); ?><br><small><?php echo e($enrollment['modalidade_nome']); ?></small></td><td><?php echo e($enrollment['temporada_nome']); ?></td><td><div class="dashboard-course-enrollment-status"><strong><?php echo e($enrollment['status_label']); ?></strong><button type="button" class="btn btn-secondary dashboard-course-enrollment-details-open" data-details="<?php echo $enrollmentDetailsJson; ?>">Ver detalhes</button></div><?php if (!empty($enrollment['temporada_encerrada'])) { ?><br><small class="muted">Temporada encerrada</small><?php } ?><?php if ($enrollment['status'] === 'lista_espera' && !empty($enrollment['posicao_lista_espera'])) { ?><br><small class="muted">Posição na lista: <?php echo e((string) $enrollment['posicao_lista_espera']); ?></small><?php } ?><?php if (!empty($enrollment['orientacao'])) { ?><br><small><?php echo e($enrollment['orientacao']); ?></small><?php } ?></td><td><div class="dashboard-course-enrollment-actions"><?php if ($canCancel) { ?><form method="POST" action="<?php echo e(url('/cursos/inscricoes/cancelar')); ?>" data-dashboard-enrollment-cancel="1" data-manual-submit="1" data-person-name="<?php echo e((string) $enrollment['nome_completo']); ?>" data-class-name="<?php echo e((string) $enrollment['turma_nome']); ?>"><input type="hidden" name="inscricao_id" value="<?php echo e((string) $enrollment['id']); ?>"><button type="submit" class="btn btn-secondary">Cancelar</button></form><?php } ?><?php if (!empty($professorEnrollmentDeletionEnabled)) { ?><form method="POST" action="<?php echo e(url('/cursos/inscricoes/excluir-definitivamente')); ?>" data-dashboard-enrollment-delete="1" data-manual-submit="1" data-person-name="<?php echo e((string) $enrollment['nome_completo']); ?>" data-class-name="<?php echo e((string) $enrollment['turma_nome']); ?>"><input type="hidden" name="inscricao_id" value="<?php echo e((string) $enrollment['id']); ?>"><button type="submit" class="btn btn-danger">Excluir definitivamente</button></form><?php } ?><?php if (!$canCancel && empty($professorEnrollmentDeletionEnabled)) { ?><span class="muted">-</span><?php } ?></div></td></tr>
            <?php } ?>
        </tbody></table></div>
    <?php } ?>
</section>
...
<div id="dashboard-course-enrollment-delete-modal" class="popup-overlay hidden" aria-hidden="true">
    <div class="popup-card course-enrollment-info-card" role="dialog" aria-modal="true" aria-labelledby="dashboard-course-enrollment-delete-title">
        <div class="popup-head"><h3 id="dashboard-course-enrollment-delete-title">Excluir inscrição definitivamente</h3><button type="button" class="popup-close-icon" data-dashboard-enrollment-delete-close="1" aria-label="Fechar confirmação">&times;</button></div>
        <div class="popup-body"><p id="dashboard-course-enrollment-delete-question"></p><div class="alert-inline"><strong>Atenção:</strong> a inscrição e seu histórico serão removidos definitivamente. A exclusão será bloqueada se houver alguma chamada registrada.</div></div>
        <div class="popup-actions"><button type="button" class="btn btn-secondary" data-dashboard-enrollment-delete-close="1">Manter inscrição</button><button type="button" class="btn btn-danger" data-dashboard-enrollment-delete-confirm="1">Sim, excluir definitivamente</button></div>
    </div>
</div>
</div>
