    ...
    public function classCopyOptions(): void
    {
        ...
        try {
            ...
            if ($stage === 'temporadas') {
                ...
                $payload = ['items' => $service->sourceClassesForDestinationFilters(trim((string) ($_GET['temporada_origem'] ?? '')), (int) ($_GET['modalidade_destino_id'] ?? 0), (int) ($_GET['local_destino_id'] ?? 0))];
            } elseif ($stage === 'detalhe') {
                $payload = $service->copyData(trim((string) ($_GET['temporada_origem'] ?? '')), (int) ($_GET['turma_origem_id'] ?? 0), (int) ($_GET['temporada_destino_id'] ?? 0), (int) ($_GET['modalidade_destino_id'] ?? 0), (int) ($_GET['local_destino_id'] ?? 0));
            } else {
